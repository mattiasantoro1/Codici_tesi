...
int_qmc={};  int_cqmc={};   abs_err_cqmc={};    abs_err_qmc={};
for deg=degV
    ...
    abs_err_cqmc{end+1}=abs(i_cqmc-2.308049752574254);
    abs_err_qmc{end+1}=abs(i_qmc-2.308049752574254);
end
...
switch operation_parm
    ...
    %case 4
        %operation_str='Symmetric difference';
end
disp(operation_str);
fprintf('\n \t CARDINALITY HALTON SET       : %7.0f',card);
fprintf('\n \t CARDINALITY QMC (IN DOMAIN)  : %7.0f',size(XY,1));
fprintf('\n \t BOX                          : [%1.2g,%1.2g]x[%1.2g,%1.2g]',...
    min(bbox.Vertices(:,1)),max(bbox.Vertices(:,1)),min(bbox.Vertices(:,2)),max(bbox.Vertices(:,2)))
area_bbox=abs(max(bbox.Vertices(:,1))-min(bbox.Vertices(:,1)))*abs(max(bbox.Vertices(:,2))-min(bbox.Vertices(:,2)));
